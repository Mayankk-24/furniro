...
function AddToCart({ onClose }) {
    ...
    const handleDelete = async (d_id,p_name) => {
        ...
        if (res.status === 200) {
            const updatedProduct = AddedProducts.filter((item) => item?.productId !== d_id);
        ...
        <>
            <div className='h-[746px] w-[417px] bg-white absolute top-0 right-0 z-50 border border-gray-300 overflow-hidden'>
                <div className='flex items-center justify-between'>
                    ...
                    <TbLockX size={19} color='#9F9F9F' className='mr-7 -translate-y-3' onClick={onClose} />
                </div>
                {
                    Loading ? (
                                    ...
                                    return (
                                        <div key={index} className='flex items-center px-6 mb-6'>
                                            ...
                                            <div className='w-[60%] flex items-center justify-between ml-8'>
                                                <div className='mr-10'>
                                                    <h2 className='text-base text-black font-normal'>{product.name}</h2>
                        ...
                        (
                            <div className='flex flex-col items-center justify-center pt-28'>
                                ...
                                <div>
                                    ...
                                    <p className='text-center text-[#919EAB] text-sm font-medium mb-4'>Look like you have no items in your shopping cart.</p>
                                </div>
                            </div>
                        )
                }
                <div className='absolute bottom-0 z-10 bg-white'>
                    <div className='flex items-center p-7 justify-between mr-16'>
                        ...
                        <span className='text-base font-semibold text-primary'>Rs. {' '}
                            {AddedProducts && AddedProducts?.reduce((total, item) => total + item.price * item.quantity, 0).toLocaleString()}
                        </span>
                    </div>
                    <div className='border-t-2 p-5 flex items-center gap-5'>
                        <button className='bg-white text-black rounded-[50px] hover:text-black  border-2 py-2 px-8 text-sm border-black ' onClick={() => AddedProducts.map((item) => handleCart(item._id))}>Cart</button>
                        <Link to={'/checkout'}><button className='bg-white text-black rounded-[50px] hover:text-black  border-2 text-sm py-2 px-6 border-black'>Checkout
                        </button></Link>
                        <Link className='bg-white text-black hover:text-black rounded-[50px] py-2 px-3 border-2 text-sm  border-black' to={'/compare'}>Comparison</Link>
                    </div>
                </div>
            </div>
        </>
